...
an external memory data structure, and thus it optimizes the number of memory transfers
performed rather than the number of CPU instructions executed. Unlike
other external memory data structures, like the widely used B-tree, the CO
B-tree does not have access to the parameters of the machine's memory
hierarchy, such as the block size/capacity at each level, or the relative
speeds. Hence, the CO B-tree is \textit{cache-oblivious}.  Despite this, the CO
B-tree achieves the same provably optimal search bound as the B-tree
and near optimal insertion and deletion bounds. In this paper we first describe
the original CO B-tree and explain how it achieves its performance bounds.
...
at the expense of the search bound; and the \textit{cache-oblivious
...
Most external memory algorithms assume an idealized model of computation in which
memory is a flat byte-addressable array that can be read and modified in some
fixed time. However, in practice memory is typically
organized as a hierarchy consisting of fast on-chip caches (L1, L2, etc.),
slower DRAM, and even slower disks. When a program requests data, the data is
located in the memory hierarchy and typically carried up the chain to the
fastest memory layer (closest to the CPU), which is usually a cache. Unfortunately, this
cache has limited capacity, and once it is full any new data
...
exponentially long time, and access times between the different layers can differ
...
External memory algorithms are wary of this fact, and focus on optimizing the
number of memory transfers performed instead of the number of CPU instructions
executed. They often try to exploit the fact that memory hierarchies transfer
data in blocks rather than individual machine word units. These
algorithms are designed to limit the number of memory transfers, as well as
preserve the locality of related data to amortize costly memory accesses.

\subsection{B-trees}

Because external memory algorithms are primarily concerned with data transfers,
many external memory problems can be reduced to finding
a suitable data structure that efficiently implements certain operations. A well
known external memory data structure is the B-tree \cite{BayerM72}, which is 
widely used in databases and file-systems. The B-tree
works by making every node in the tree the same size as a memory block in a
particular level of the hierarchy. The fanout of the tree
is also proportional to the block size.  The
...
However, in practice, architectures typically have more than two levels in their memory hierarchies.
The argument for the DAM model is that in practice, only one
level in the memory hierarchy tends to dominate the performance, and thus a
two-level memory model is sufficient.  However, this means that B-trees may
incur an increasing number of memory transfers if the dominating level
...
that needs to be modified for every level in the memory hierarchy. This tuning
can be very difficult and error prone, and may
...
about the memory hierarchy, and knows nothing about the speed or the
...
algorithms should gracefully scale to multi-level hierarchies as well.
...
it does not require explicit information about the memory hierarchy.
...
B-tree and explain how it works. This includes a detailed description of two data
structures integral to the CO B-trees performance: the
packed memory array (PMA) and strongly weight-balanced binary search tree
(SWBST). Following this
discussion we analyze three variants of the CO B-tree: the CO string B-tree (a CO B-tree with
...
The CO B-tree optimizes the number of memory transfers by organizing its
...
The vEB layout can be shown to use $O(\log_{B}{N})$ memory transfers for 
searches \cite{veb1, veb2}. Thus, using just the vEB layout, the CO
B-tree has already achieved the desired bound for search queries. However, it still
...
tree} (SWBST) is used \cite{swbst}.  Let the weight of a node $u$ be equal to the
total number of $u$'s descendants plus 1. A recursive definition for the weight of $u$ is the sum
of the weights of $u$'s children plus 1. Formally, $w(u) = 1 +
...
Let us now consider insertions. To insert an element, simply search through
the tree to find where the element would go. After inserting the element into some node $w$, 
...
every node along the path between that ancestor and $w$ will also be unbalanced.
This requires splitting nodes to restore balance, starting from $w$
...
Deletions are supported similarly, except we check if a node $u$ has a weight $w(u)
< \frac{c^{h-1}}{2}$. In this case, we merge two neighbors together and proceed
up the tree. However, the merging procedure may produce a new node that has a 
weight larger than the bounds
...
Assuming the elements are now organized in a SWBST, we will show how to
of map this tree to memory and explain how it achieves the desired number
of memory transfers per operation. Do do this, we will use a packed memory 
array (PMA)\cite{BenderDemainColton}. There are three main costs
that the PMA addresses: \textit{inserts}, \textit{splits/merges}, and
\textit{pointer updates}.

First, the PMA needs to maintain free space for future inserts.
%Doing so may seem contradictory: maintaining the locality of elements while keeping space available for new inserts. 
To achieve a balance between maintaining locality 
and ensuring free space for new items, the PMA uses a technique
described in \cite{packedmemoryarray}.
The PMA performs an insertion into the SWBST using only $O(1 +
(\log^2{N}) / B)$ memory transfers. Note that this is the cost of updating the memory
layout of the SWBST in the PMA, not the cost of updating the SWBST%data structure
...
that each node has $\Theta(c^h)$ descendants (recall that this is the key property
that SWBST provides, and is shown in Equation~\ref{eq:bounds}). Thus, merging 
or splitting a node will cause $\Theta(c^h)$ nodes to move, requiring 
$O(1 + c^h/B)$ memory transfers. It follows from the definition of SWBST in 
\cite{swbst} that when a node is rebalanced, it will require $\Theta(c^h)$ insertions 
or deletions until it is rebalanced again. Thus, the amortized cost of each insertion 
and/or deletion is $O(1/B)$. When a node is split, it may traverse all the way up the 
tree, and thus the split/merge bound is $O((\log N) / B)$ memory transfers.

Third, the PMA needs to update the parent and child pointers that
become invalidated as a result of the movement of the nodes. If the pointers are
all packed together in memory (e.g. within $B$ distance of each other) then
only $O(1)$ memory transfers are required. However, a problem arises if they 
are further apart. To address this issue, buffer nodes are introduced.  Buffer
nodes ``stretch'' the vEB layout so that nodes do not move far away from one
another. Using buffers, the PMA is able to achieve a pointer update cost
using $O(1+ \frac{\log{B}}{\sqrt{B}}\log^2{N})$ memory transfers.

Of these three terms, the cost of maintaining space for newly
inserted elements dominates - $O((\log^2{N})/B)$. Therefore, overall the CO B-tree
...
While B-trees achieve an optimal bound in terms of memory transfers for
...
linked list of buffers, compared to use of just a single buffer in the BRT. Each of
these buffers is another shuttle tree. A shuttle tree node bounds both the length 
of its linked list, and the overall height (and thus capacity) of each of the 
buffered shuttle trees contained within its linked list.
...
bounding its height. The buffers are arranged in increasing order of size from
left-to-right across the linked list. Thus, a node's first buffer is smaller than its last
...
\Search{} and \Scan{} work mostly the same as their counterparts in the original CO 
B-tree. The only difference is that a node's buffer must be searched before continuing
down the tree.

\Insert{} also behaves like insertions into the original CO B-tree, except that 
elements are buffered as they are inserted. \Insert{} starts by moving 
down the tree from the root to the leaves, and stopping when a node
that has a buffer is located. Note that because of the
nature of Fibonacci numbers, a node may not have any buffers. 
The element is inserted into the smallest buffer in the node's linked
list. If this insertion causes the chosen buffer to overflow (e.g. the height of the
...
overflows, all of its elements are \textit{shuttled} down the tree.
Eventually, a leaf node is reached and the insertion may invalidate the balance
constraints on the SWBST. The shuttle tree rebalances
itself as described previously in Section~\ref{sec:structure}: the leaf node is 
split into two and this split cascades up the tree until balance is restored.
%% TODO: explain what it means to be "shuttled" down the tree

The shuttle tree also needs to manage buffers when a node is split.
When a node $u$ is split into two nodes $u_1$ and $u_2$, the shuttle tree
immediately allocates enough space for all of $u_2$'s buffers. $u_1$
reuses the space allocated for $u$'s buffers. Then a single scan is performed
over $u$'s buffers, and the elements are placed in the new buffers
accordingly.

The PMA structure described in Section~\ref{sec:layout} is used to: maintain
room for inserts, bound the costs of merging and splitting, and bound the 
costs of updating parent and child pointers in the shuttle tree
...
key observation is that when you visit a node you only search its largest buffer
(the last buffer in the linked list for that level). The smaller buffers will eventually
be visited as you recurse down the tree. Eventually, you will traverse
down to a single subtree that fits in a single block, and thus can be
searched with one memory transfer. Since the fanout is proportional to the
block size, it will only take $O(\log_B N)$ block accesses to reach this
subtree. Thus, by induction, the worst case runtime is $O(\log_B N)$.
...
memory transfers. This improvement over the CO B-tree is achieved using
the shuttle tree's buffers to maintain locality of the elements long
...
tree, not just the single element that caused the overflow. Thus, the cost of
insertion is amortized by the number of elements in a node's largest
...
required to find the starting element of the range. Once the first
element's block is transferred, the shuttle tree proceeds by traversing the
tree and scanning the elements in order. Since the vEB layout enforces
spatial locality of nearby nodes in the tree, the shuttle tree only has to read
in a new block roughly once every $B$ elements. Thus, each individual memory
access is is amortized to $1/B$ memory transfers, and the total number of 
memory transfers is $O(L/B)$.

Note that the shuttle tree uses $O(N)$ space, since 
the majority of the buffers are very small and thus do not impact the 
overall size of the tree.

\subsection{COLA}

The cache-oblivious lookahead array (COLA) is notably different from the other
CO data structures discussed thus far. Instead of an explicit tree
...
This array has size $2^0 = 1$, thus the element is in its own array.
Each time an element is inserted, check to see if there is another array of
...
array that is twice the size and thus is moved into the next level. The COLA
...
COLA. Since all the arrays are sorted the COLA can use a binary search to find the
...
number of required memory accesses. Every 8th element from the $(k+1)$-st level
exists in the $k$-th level array in its proper sorted location. This element
also contains a pointer to its real location in the $(k+1)$-st level. Call this
pointer a \textit{real lookahead pointer}. Additionally, every
4th cell in a $k$-th level array contains two pointers pointing to the next and
...
COLA performs a binary search within an appropriate range, determines the range
...
times, since at that point it will be in the last array.
...
element is $1/B$. Thus it follows from our earlier assumption that the 
overall amortized number of memory transfers needed to insert an element is 
$O((\log N)/B)$.

\Search{} runs in $O(\log N)$ memory transfers. The proof shows that
the COLA never needs to search more than 8 elements on each
level. Since there are $O(\log N))$ levels and 8 is a constant, it
follows that overall search uses $O(\log N)$ memory transfers. The fact that
only 8 elements need to be searched per level is shown through induction.
The base case is trivial. The first three levels of the COLA each have at most
8 elements (note that these elements are either real elements or
lookahead pointers). The COLA can search these levels with no more
than 8 member transfers. As the COLA follows lookahead pointers to the
higher levels, it will eventually reach a level where $k \geq 3$. Note that
the COLA does not ``enter'' this level at the start of the array. It
arrives there by following the real lookahead pointers from the levels below,
and thus is located at some index $i$ that contains an element $e_i$ such that $e_i <
e$ and $e$ is the element being searched for. Now we want to
find a new $i$ such that $e_i \leq e < e_{i+1}$. Once this $i$ is found, the
duplicate lookahead pointers are used to locate the \textit{next} and
...
each level in the COLA, every 8 element in a level is present in the
previous level. Thus, it follows that in the next level only 8 elements need
to be searched. Furthermore, since the current level was reached from a lower
level, it follows that only 8 elements need to be searched on this level to
find the appropriate $i$. Therefore, overall, the COLA uses $O(8\log{N}) =
...
scheme, which uses less space and improves compressed data locality.

\subsubsection{General COSB-tree Structure}

The dynamic COSB-tree consists of three layers: a weight-balanced centroid tree stored in
a packed-memory array (PMA, see Section~\ref{sec:layout}); a PMA of hash data for the keys, known as the 
\emph{hashdata} layer; and a PMA of compressed values for the original key,
known as the 
\emph{keydata} layer (see Figure~\ref{fig:string_structure}). Each leaf of the centroid tree contains a pointer into
the hashdata array. The centroid tree contains only hash values, and is designed such that
each leaf points to the respective \emph{neighborhood} of keys being
...
centroid trees. Figure~\ref{fig:centroid} depicts an example centroid tree from
\cite{BenderFaKu06}.
...
trie resulting from excluding $y$ and its subtree, and call this the \emph{up trie} of
$y$. The \emph{centroid tree} is obtained by rooting the tree at the centroid vertex
$\rho$, and making its children recursively defined centroid trees of its up and
down tries. 

The centroid tree in terms of structure is essentially a weight-balanced binary tree (see
Section~\ref{sec:structure}), and is laid out in memory using a modified vEB
layout (see Figure~\ref{fig:veb}). However, instead of using height $h/2$ as
described previously, the \emph{hyperhyperfloors} of the weights of nodes are
used as a guide for cutting the tree into equal-size subtrees for the vEB layout. The
\emph{hyperhyperfloor} of x refers to rounding x down to the nearest power of a 
power of 2.

In addition, a different approach is used to rebalance the centroid tree: put all the
...
Similar to the original CO B-tree, the centroid tree is stored in a PMA
(see Section~\ref{sec:layout}). However, the centroid tree PMA is more 
flexible with a modified updating scheme. The changes made to updating 
"windows" into the array are as follows (see Section~\ref{sec:layout} 
for more on "window" updates): on insert or 
delete, the area around the update point is scanned for sections
or windows of the array to rearrange. If a window of a
certain \textit{density} threshold is found (as described previously in~\ref{sec:layout}), the 
nodes are evenly distributed across the
window to make room for more nodes. To ensure that updating pointers is not too
expensive, any node included in the window must have all of its subtrees
included. The subtrees are dictated by the vEB layout, and subtrees are chosen
from the bottom up. The smallest neighboring subtrees (single nodes) in the vEB
layout are added first, and larger subtrees are included when all of their own
subtrees have already been added to the section. top subtrees are only added after all
bottom subtrees are added (in terms of the vEB layout). Figure
...
transfers per update. In conjunction with the cost of rebalancing described above,
...
which each subtree is added to the window to be rearranged.}
\label{fig:centroid_relayout}
\end{figure}

\subsubsection{Modifying Front Compression}

This section provides a brief summary of front compression, and how it is modified to
optimize compression and decoding for dynamic COSB-trees. This approach 
increases the locality of the compressed data.

First, front compression is described: consider a sequence
...
front compressing $D$ to $O(1+||k||B)$ memory transfers, while 
maintaining a compressed size of
...
can be decoded with just the last $c||k_{i}||$ characters in the compression stream, add
$\pi_{i}$, $\sigma_{i}$ as described above for standard front compression.
Otherwise, add $k_{i}$ to the stream
...
$k$ and call it $\rho$. Traverse the centroid tree of the COSB-tree using
$\rho$ to obtain the successor $\rho_{s}$ and predecessor $\rho_{p}$ of $k$.
Index into the hashdata layer with the resulting pointer from the centroid tree traversal,
and perform a sequential scan across the PMA
from $\rho_{p}$ to $\rho_{s}$ to find $\rho$. On a match with hash value
...
$\rho_{e,s}$. Only start noting the beginning of the range when the first key with
...
set of keys in $D$ that are within the given range.

\Insertkonly{} and \Delete{} require $O(1+\log_{B}N+\log^{2}N||k||/B)$
memory transfers (w.h.p.). As mentioned previously in Section~\ref{sec:centroid_trees}, 
due to rebalancing updates have amortized cost of $O(1+(\log^{2}N)/B)$ memory transfers.
The rest of the cost for insertions and deletions is due to having to find the correct key to
...
without modification. However, this approach yields poor performance.
...
B-trees, first presented in \cite{BenderFiGi05}. Bender et al. develops two different
data structures to solve this problem: first an exponential CO B-tree,
and second a CO B-tree based on a PMA (see Section~\ref{sec:layout} for more on PMA's). Bender et al. also
present a lock-free variant of the latter data structure.

\subsection{Description}

Before describing the data structures in more detail, we first consider
the concurrency model used throughout \cite{BenderFiGi05}.
...
is chosen for the data structure. Each node in the tree maintains the keys
which divide up its children, in addition to a \textit{right-link} pointer. The right-link
pointer contains a pointer to the node's right sibling, and the key value of
...
very little modification). However, the PMA variant of CO
...
that $k$ will be inserted into. A lock is acquired on that node, $k$ is
...
is split (leaf nodes have $h = 0$). To perform a split, we first reacquire a
lock on the node to be split. We split the node into $u$ and $u'$, where $u$
contains all the keys $< k$, and $u'$ contains the keys $\geq k$. We then release
...
split, we reuse the original node in constructing $u$. Doing so ensures that
...
(w. h. p.). The probabilistic bound comes from the probabilistic
...
\Omega(\log_{\alpha}{\log{\log{N}}})$, the total number of keys in the
tree is $O(2^{\alpha^h})$. The proof is then completed by summing over the
range of $h$ which is probabilistic $[0, O(\log_{\alpha}{\log{N}})]$, which is the
the number of memory transfers made, starting from some level $h$,
to find the correct child.

Sequential insert takes $O(\log_B{N} + \log{\alpha}{\log{B}})$ block
transfers (in expectation). This proof is similar to the search proof: 
consider inserting a tree at some height $h$. By noting that
the probability that a particular node reaches height $h$ is
$2^{-(\alpha^h-1)/(\alpha-1)}$, a summation over all levels of the cost of
rebuilding a node yields the desired bound.

\subsection{Packed Memory Array CO B-tree}
\label{sec:pma}

The packed-memory array (PMA) CO B-tree is a two level data structure, containing a static CO B-tree
which indexes into a PMA (see Section~\ref{sec:layout} for more on PMA's). 
Bender et al. introduce a new PMA, 
called a \textit{one-way packed-memory array}, which he
argues is easier to manage concurrently.

\subsubsection{One-Way PMA Primer}

A one-way PMA is a contiguous memory structure which has three
...
N)$-region of the PMA that would contain $k$. Let $s$ be the slot
such that slot $s - 1$ contains the largest key smaller than $k$.
Lock the current region and scan right to find $s$. If scanning
...
technique). Once $s$ is located, if $s$ is free, just insert
$k$. Otherwise, a \textit{rebalance} operation is required.

To perform a rebalance operation, we scan right (locking as necessary) until the
region from $s$ to the current position $s'$ is not "too dense". The
...
Sequential search memory transfer bounds are not presented in the paper, but presumably
...
on an accounting argument that places $\Theta((\log^2{m})/B)$ dollars in each
array slot. The proof is not presented in any detail in \cite{BenderFiGi05}, but
...
The lock-free variant is based on the PMA CO B-tree, except the locks acquired
during the PMA modification are replaced by augmenting each element in the PMA
with a marker. The marker is used to denote the ongoing (concurrent) operation of
...
ever encounters a cell with a marker, it temporarily suspends its current task and
helps to complete the pending operation. When the originating process is
context-switched back in, any store-conditionals (SCs) will fail, and then the process can check to
see if the operation has already been completed. The implementation of the
non-blocking primitives is described in more detail in \cite{BenderFiGi05},
...
done. Otherwise, a rebalance is necessary. The rebalance proceeds as
...
% are we putting this in in the final copy?
%\section{Acknowledgments}
%We sincerely thank Dr. David Karger and all of the TAs for 6.854 
f%or their guidance, help, and support. May the Flow (Max-Flow, that is) be with you all.
